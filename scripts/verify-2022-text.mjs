#!/usr/bin/env node
/** Parse 2022 text spreadsheet, compare to JSON/DB, optionally reload. */
import fs from "fs";
import os from "os";
...
const TEXT = fs.readFileSync(path.join(__dirname, "data", "2022-spreadsheet.txt"), "utf8");
...
if (process.argv.includes("--fix")) {
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        seasonYear: SEASON,
        adjustments: expected.map(({ dbPlayerName, ...r }) => r),
      },
      null,
      2,
    ),
  ...
  if (dbRows.length) {
    ...
    console.log("Deleted existing 2022 historical rows.");
  }
}
